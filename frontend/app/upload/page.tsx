...
export default function UploadPage() {
  ...
  return (
    <main className="pt-6">
      ...
      {step === "extracting" && <p className="mt-8 text-center text-sm text-ink/60">Reading your report…</p>}

      {step === "confirm" && (
        <div className="mt-4 space-y-4">
          <p className="rounded-xl bg-white p-3 text-sm shadow-sm">
            Is this right? Check the values before they go on your timeline.
            {provider === "offline-parser"
              ? " (Read on-device - add API keys for full AI reading.)"
              : ` (Read by ${provider}.)`}
            {language && language !== "en" && ` Detected language: ${language}.`}
          </p>
          {extractMessage && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">{extractMessage}</p>}
          {values.map((v, i) => (
            <div key={v.code}
              ...
              <label className="flex items-center gap-2 text-sm font-semibold">
                ...
                {v.label}
                {v.confidence < 0.7 && <span className="text-xs font-normal text-amber-700">please double-check</span>}
              </label>
              <div className="mt-2 flex items-center gap-2">
                ...
                {v.reference_range && <span className="ml-auto text-xs text-ink/50">ref: {v.reference_range}</span>}
              </div>
          ...
          <div className="flex gap-3">
            <button onClick={reset} className="flex-1 rounded-xl border border-ink/15 bg-white p-3 text-sm font-semibold">Cancel</button>
            <button onClick={onConfirm} disabled={values.length === 0}
              className="flex-1 rounded-xl bg-brand p-3 font-semibold text-white disabled:opacity-40">Save to timeline</button>
          </div>
        </div>
      )}
      ...
      {step === "done" && (
        <div className="mt-4 space-y-4">
          <p className="rounded-xl bg-white p-3 text-sm text-brand shadow-sm">✅ {savedCount} value{savedCount === 1 ? "" : "s"} saved to your timeline.</p>
          {flags.length > 0 && (
            <div className="space-y-2">
              {flags.map((f) => (
                <p key={f.code} className="rounded-xl bg-red-50 p-3 text-sm text-red-800 ring-1 ring-red-200">
                  ⚠️ {f.label}: {f.value} {f.unit}. {f.message}
                </p>
              ))}
              <p className="text-xs text-ink/60">Your doctor will see {flags.length === 1 ? "this flag" : "these flags"} in the review queue.</p>
            </div>
          )}
          <div className="flex gap-3">
            <a href="/" className="flex-1 rounded-xl bg-brand p-3 text-center font-semibold text-white">View timeline</a>
            <button onClick={reset} className="flex-1 rounded-xl border border-ink/15 bg-white p-3 text-sm font-semibold">Upload another</button>
          </div>
        </div>
      )}
